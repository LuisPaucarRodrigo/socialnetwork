<template>
    ...
    <MyTransition :transitiondemonstration="showDocs">
        <div class="relative">
            ...
            <Link class="w-full" :href="route('documment.management.folders')"
                >CCIP</Link
            >
        </div>
    </MyTransition>
        ...
        <div class="mb-4">
            ...
            >
                <Link
                    ...
                    :href="
                        route('archives.show', {
                            folder: item.archive.folder_id,
                        })
                    "
                >
                    <div class="flex items-center">
                        <svg
                            class="w-4 h-4 mr-2 text-red-600 dark:text-red"
                            aria-hidden="true"
                            xmlns="http://www.w3.org/2000/svg"
                            fill="currentColor"
                            viewBox="0 0 20 20"
                        ...
                        <span>{{
                            item.archive.folder.name +
                            "/" +
                            item.archive.name +
                            "." +
                            item.archive.extension
                        }}</span>
                    </div>
                </Link>
            </MyTransition>
            <MyTransition
                v-for="item in archiveAlarms7"
                :key="item.id"
                class="ml-4"
                :transitiondemonstration="showArchivesAlarms"
            >
                ...
                >
                    <div class="flex items-center">
                        <svg
                            class="w-4 h-4 mr-2 text-yellow-600 dark:text-yellow"
                            aria-hidden="true"
                            xmlns="http://www.w3.org/2000/svg"
                            fill="currentColor"
                            viewBox="0 0 20 20"
    ...
    <MyTransition :transitiondemonstration="showDocs">
        <div class="relative">
            <Link class="w-full" :href="route('local.drive.index', { root: 1 })"
                >Local Drive</Link
            >
        </div>
    </MyTransition>
    <MyTransition
        ...
        :transitiondemonstration="showDocs"
    >
        <Link
            class="w-full"
            :href="route('documment.management.folders.validation')"
            >Aprobación</Link
        >
    </MyTransition>
</template>
