      ...
      <AuthenticatedLayout>
        <div class="flex space-x-4">
          ...
          <div class="flex items-center">
            ...
            <select v-model="selectedSection" id="selectElement" class="rounded-md py-2 text-sm text-black hover:bg-gray-100 border-indigo-600">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-4 mt-5">
          ...
          <div v-for="document in filteredDocuments" :key="document.id" class="bg-white p-4 rounded-md shadow md:col-span-2">
            ...
            <div class="flex items-center">
              ...
              <button @click="downloadDocument(document.id)" class="flex items-center text-blue-600 hover:underline" style="margin-left: 1%;">
                <ArrowDownIcon class="h-4 w-4 ml-1" />  
              </button>
              <button @click="openPreviewDocumentModal(document.id)" class="flex items-center text-green-600 hover:underline" style="margin-left: 2%;">
        ...
        <teleport to="body">
          ...
          <div v-if="isPreviewDocumentModalOpen" class="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div class="modal-container flex items-center justify-center">
              <div class="modal-content bg-white p-5 rounded-md relative" style="width: 100% !important; height: 60% !important;">

                <button @click="closePreviewDocumentModal" class="close-button absolute top-0 right-0 mt-2 mr-2">&#10006;</button>
                <!-- Contenido del modal -->
                <iframe :src="getDocumentUrl(documentToShow)" width="100%" height="100%"></iframe> <!-- Cambiado a 100% de ancho y alto -->
              </div>
            </div>
          </div>
        </teleport>
          ...
          <div v-if="isCreateDocumentModalOpen" class="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div class="modal-container flex items-center justify-center">
              <div class="modal-content bg-white w-1/2 p-5 rounded-md relative">
                <button @click="closeCreateDocumentModal" class="close-button absolute top-0 right-0 mt-2 mr-2">&#10006;</button>
                <form @submit.prevent="submitForm" class="mt-5">
                    <label for="documentFile" class="text-gray-700">Documento:</label>
                    <InputFile @change="handleFileChange" id="documentFile" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6" />
