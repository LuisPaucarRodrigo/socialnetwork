                ...
                <div class="grid grid-cols-1 gap-x-8 gap-y-4 sm:grid-cols-2">
                    ...
                    <div class="mt-2">
                        ...
                        <div v-if="formChangelog.invoice && formChangelog.id" class="flex items-center">
                            <span>Archivo: </span>
                            <a target="_blank" :href="route('room.rental.show_invoice', {
...
function openEditChangelog(item, car) {
    ...
    formChangelog.reset();
    formChangelog.items = item.room_changelog_items?.map((i) => i.name) ?? [];
    show_plate.value = car.plate;
    show_owner.value = car.user.name;
}
...
async function submitChangelog() {
    let url = formChangelog.id
        ? route("room.rental.update_changelog", {
            car_changelog: formChangelog.id,
        })
        : route("room.rental.store_changelog", { car: car_id.value });
    let method = "post";
