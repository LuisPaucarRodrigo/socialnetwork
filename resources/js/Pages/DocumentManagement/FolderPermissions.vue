<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="min-w-full rounded-lg shadow">
                    ...
                    <form @submit.prevent="addFolderArea">
                        <div class="mt-2 mb-5">
                            <select required v-model="addFolderAreaform.area_id"
                                ...
                                <option v-for="item in makeAreasArray(permissions, upper_folder_areas)" :key="item.id"
                                    :value="item.id">
                                    {{ `${item.name}` }}
                                </option>
                            </select>
            ...
            <Modal :show="showDeleteFA" @close="closeDeleteFAModal">
                <div class="p-6">
                    <h2 class="text-lg font-medium text-gray-900 mb-2">
                        Eliminar Permisos de Área
                    </h2>
                    <InputLabel class="mb-5">
                        Al eliminar el permiso de esta área se eliminaran los permisos de todas sus subcarpeatas.
                    </InputLabel>
...
//delete folder area
const showDeleteFA = ref(false)
const folderAreaIdToDelete = ref(null)

function openDeleteFAModal (id) {
    showDeleteFA.value = true
    folderAreaIdToDelete.value = id
}
