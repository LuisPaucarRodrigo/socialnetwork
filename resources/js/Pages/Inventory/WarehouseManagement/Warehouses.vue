  ...
  <AuthenticatedLayout :redirectRoute="'warehouses.warehouses'">
    ...
    { warehouse_id: item.id })" class="text-blue-600 underline whitespace-no-wrap hover:text-purple-600">
        ...
        <div v-for="item in warehouses" :key="item.id"
          ...
          <div class="text-gray-500 text-sm">
            <div class="grid grid-cols-1 gap-y-1">
              <Link :href="item.id === 3 ? route('warehouses.purchaseorders.approve', { warehouse: item.id }) : route('inventory.retrieval_entry.index')"
                class="text-blue-600 underline whitespace-no-wrap hover:text-purple-600"> {{ item.id === 3 ? 'Compras':'Ingresos' }}</Link>
              <Link :href="item.id === 3 ? route('warehouses.products', { warehouse: item.id }) : route('inventory.retrievalProduct.index')"
                class="text-blue-600 underline whitespace-no-wrap hover:text-purple-600">Productos</Link>
              <Link :href="item.id === 3 ? route('warehouses.dispatches', { warehouse: item.id }) : route('inventory.retrievalDispatch.index')"
                class="text-blue-600 underline whitespace-no-wrap hover:text-purple-600">Despachos</Link>
            </div>
          </div>
        </div>
