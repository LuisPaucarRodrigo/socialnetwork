<template>
    ...
    <AuthenticatedLayout :redirectRoute="'warehouses.warehouses'">
        ...
        item.entry.inventory.purchase_product.name }}</p>
                                ...
                                <td class="border-b border-gray-300 bg-white px-5 py-5 text-sm">
                                    <div>
                                        <p :class="'text-green-500 whitespace-nowrap'">
                                            Aceptado -
                                            <span v-if="item.remaining_quantity === 0"
                                                class="text-green-500">Completo</span>
                                            <span v-else class="text-red-500">Incompleto</span>
                                        </p>
                                    </div>
                                </td>
                                <td v-if="hasPermission('ProjectManager')"
                                    class="border-b border-gray-300 bg-white px-5 py-5 text-sm">
                                    <button @click="() => showModalOutput(item.id,item.remaining_quantity)"
                                        ...
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            ...
                                            :class="`w-6 h-6 ${item.state && item.remaining_quantity !== 0 ? '' : 'opacity-50'}`">
                                ...
                                <!-- Expandible row -->
                                <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                                    <button :disabled="(item.project_entry_outputs.length > 0) ? false : true"
                                        type="button" @click="toggleDetails(item.project_entry_outputs)"
                                        :class="`text-blue-900 `">
                                        ...
                                        <table class="w-full ">
                                            ...
                                            <tbody>
                                                <tr v-for="(subItem, i) in item.project_entry_outputs" :key="subItem.id"
                                                    class="bg-gray-100">
                                                    <td
                                                        class="border-b w-auto text-center border-gray-200 bg-white px-5 py-5 text-sm">
                                                        {{ i + 1 }}
                                                    </td>
                                                    <td
                                                    ...
                                                    <td v-if="auth.user.role_id === 1"
                                                        class="border-b text-center border-gray-200 bg-white px-5 py-5 text-sm">
                                                        <button @click="() => deleteOutput(subItem.id)" type="button"
                                                            class="text-blue-900 ">
            ...
            <form class="p-6" @submit.prevent="submit">
                ...
                <div class="grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-6 mt-2">
                    <div class="sm:col-span-6">
                        <div class="flex">
                            <p class="mt-1 text-sm text-black-600">Cantidad solicitada: </p>
                            <p class="mt-1 ml-1 text-sm text-gray-600">{{ requested_quantity }}</p>
                        </div>
