            ...
            <table class="w-full whitespace-no-wrap">
                ...
                <tbody>
                    <tr v-for="purchase in purchases.data" :key="purchase.id" class="text-gray-700">
                        ...
                        <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                            <p :class="(purchase.state == 'Rechazado'? 'text-red-500': 'text-gray-900')+' whitespace-no-wrap'">{{ purchase.state }}</p>
                            ...
                            <div class="flex space-x-3 justify-center">
                                ...
                                @click="edit_purchase_request(purchase.id)">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" :class="'w-6 h-6 '+(purchase.state == 'pendiente' ? 'text-amber-400':'text-amber-200')">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                    </svg>
                                </button>
                                <button type="button" @click="confirmPurchasesDeletion(purchase.id)"
                                :disabled="purchase.state == 'pendiente' ? false : true"
                                    class="whitespace-no-wrap">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" :class="'w-6 h-6 '+(purchase.state == 'pendiente' ? 'text-red-500':'text-red-200')">
                                        <path stroke-linecap="round" stroke-linejoin="round"
...
const edit_purchase_request = (purchase_id) => {
    router.get(route('projectmanagement.purchases_request.create', { project_id:props.project_id, purchase_id: purchase_id }));
}
