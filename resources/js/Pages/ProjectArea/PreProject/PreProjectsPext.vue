<template>
    ...
    <AuthenticatedLayout :redirectRoute="{ route: 'preprojects.index', params: { type } }">
        <template #header>
            Anteproyectos PEXT {{ preprojects_status !== null
        ...
        <div class="min-w-full p-3 rounded-lg shadow">
            <div class="mt-6 flex items-center justify-between gap-x-6">
                <div class="flex space-x-2">
                    ...
                    <Link v-if="preprojects_status === null" :href="route('project.auto.pext', { type })"
            ...
            <form class="p-6" @submit.prevent="submitAssignUser">
                ...
                <div class="grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-6 mt-2">
                    <div class="sm:col-span-3">
                        ...
                        <div class="mt-2">
                            <select multiple v-model="assignUserForm.user_id_array" id="users" size="20"
                                class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
                                <option v-for="user in props.users" :key="user.id" :value="user.id"
                                    :selected="assignUserForm.user_id_array.includes(user.id)">
                                    {{ user.name }}
                                </option>
                            </select>
                        </div>
