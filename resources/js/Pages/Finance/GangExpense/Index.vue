<template>
    ...
    <AuthenticatedLayout :redirectRoute="'gangexpense.index'">
        ...
        <div class="mt-6 flex items-center justify-between gap-x-6">
            <button @click="add_gang_expense" type="button"
            ...
            <table class="min-w-full whitespace-no-wrap">
                ...
                <tbody>
                    <tr v-for="expense in expenses.data" :key="expense.id" class="text-gray-700">
                        <td class="sticky left-0 bg-amber-200 px-5 py-5 text-sm">
                            <p class="text-gray-900 whitespace-no-wrap">{{ expense.gang }}</p>
                        </td>
                        <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                            <p class="text-gray-900 whitespace-no-wrap">{{ expense.person }}</p>
                        </td>
                        <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                        ...
                        <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                            <p class="text-gray-900 whitespace-no-wrap">S/ {{ expense.price }}</p>
                        </td>
                        <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                            <p class="text-gray-900 whitespace-no-wrap">S/ {{ expense.price_without_igv }}</p>
                        </td>
                        <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                            <p class="text-gray-900 whitespace-no-wrap">{{ expense.description_expenses }}</p>
                        </td>
                        <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
