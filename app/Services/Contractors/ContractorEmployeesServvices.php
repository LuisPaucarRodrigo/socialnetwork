...
use App\Models\Contractors\ContractorAddress;
use App\Models\Contractors\ContractorContract;
use App\Models\Contractors\ContractorCostLine;
use App\Models\Contractors\ContractorEducation;
use App\Models\Contractors\ContractorEmergency;
use App\Models\Contractors\ContractorEmployee;
use App\Models\Contractors\ContractorExternalEmployee;
use App\Models\Contractors\ContractorFamily;
use App\Models\Contractors\ContractorHealth;
use Carbon\Carbon;
...
class ContractorEmployeesServvices
{
    ...
    {
        $query = ContractorEmployee::select('id', 'name', 'lastname', 'dni', 'phone1', 'cropped_image')->with('contract.cost_line')->whereHas('contract', function ($query) use ($state) {
    ...
    {
        $cost_line = ContractorCostLine::all();
        return $cost_line;
    }
    ...
    {
        $contract = ContractorContract::where('employee_id', $id)->first();
        DB::beginTransaction();
        try {
    ...
    {
        $employee = ContractorEmployee::with('contract.cost_line', 'education', 'address', 'emergency', 'family', 'health')->find($id);
        return $employee;
    }
    ...
    {
        $reentry = ContractorContract::find($id);
        $reentry->update([
    ...
    public function happy_birthday()
    {
        ...
        $employees = ContractorEmployee::select('id', 'name', 'lastname', 'birthdate')
    ...
    {
        $filteredFields = array_diff((new ContractorEmployee())->getFillable(), ['cropped_image']);
        ...
        $employee = ContractorEmployee::updateOrCreate(
            ['id' => $employee_id],
            $validateData
        );
        return $employee->id;
    ...
    {
        ContractorContract::updateOrCreate(
    ...
    {
        $filteredFields = array_diff((new ContractorEducation())->getFillable(), ['curriculum_vitae']);
        $validateData = $request->only($filteredFields);
        ...
        ContractorEducation::updateOrCreate(
            ['employee_id' => $employee_id],
            $validateData
        );
    }
    ...
    {
        ContractorAddress::updateOrCreate(
            ['employee_id' => $employee_id],
            $validateData
        );
    }
    ...
    {
        ContractorHealth::updateOrCreate(
            ['employee_id' => $employee_id],
            $validateData
        );
    }
    ...
    {
        $employees = ContractorExternalEmployee::with('cost_line')->paginate();
        $employees = $this->mapProfileEmployee($employees);
        return $employees;
    }
    ...
    public function searchExternal($request)
    {
        ...
        $employees = ContractorExternalEmployee::with('cost_line')
    ...
    public function storeOrUpdateExternalEmployees($validateData, $request, $external_id): Object
    {
        ...
        $e_external = ContractorExternalEmployee::updateOrCreate(
            ['id' => $external_id],
            $validateData
        );
